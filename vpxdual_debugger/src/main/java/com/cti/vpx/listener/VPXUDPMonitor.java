...
public class VPXUDPMonitor {
	...
	private int currentAdvPort = VPXUDPListener.DEFAULT_ADV_PORTNO;

	private int currentCommPort = VPXUDPListener.DEFAULT_COMM_PORTNO;

	private int currentMsgPort = VPXUDPListener.DEFAULT_CONSOLE_MSG_PORTNO;
	...
	public VPXUDPMonitor(VPXUDPListener prent) {
	...
	public VPXUDPMonitor(VPXUDPListener prent, int[] ports) {

		this.currentAdvPort = ports[0];

		this.currentCommPort = ports[1];

		this.currentMsgPort = ports[2];
	...
	private void createDefaultMonitors() {
	...
	public void sendBoot(String ip, int processor, int flashdevice, int page) {
		...
		try {
			...
			send(buffer, ip, currentCommPort, false);
	...
	public void setPeriodicityByBroadcast(int period) {
		...
		try {
			...
			send(buffer, VPXUtilities.getCurrentInterfaceAddress().getBroadcast(), currentCommPort, true);
	...
	public void sendPeriodicity(String ips, int period) {
		...
		try {
			...
			send(buffer, recvip, currentCommPort, false);
	...
	public void sendMessageToProcessor(String ip, int core, String message) {
		...
		try {
			...
			send(buffer, ip, currentMsgPort, false);

			VPXLogger.updateLog("Message sent to " + ip + " for core " + core);
	...
	public void sendMessageToProcessor(String ip, String msg) {
		...
		send(msg.getBytes(), ip, currentMsgPort, false);
	...
	public void sendHexArrayToProcessor(String ip, int currentPacket, long totalPacket, long arrayLenth, int core,
		...
		try {
			...
			if (currentPacket < totalPacket) {
				...
				send(buffer, InetAddress.getByName(ip), currentCommPort, false);
			}
		} catch (Exception e) {
			VPXLogger.updateError(e);
			e.printStackTrace();
		...
		try {
			...
			VPXNetworkLogger
					.updatePacket(new VPXNWPacket(0, VPXUtilities.getCurrentTime(0), VPXSessionManager.getCurrentIP(),
							ip, "UDP", currentCommPort, buffer.length, new JLabel("Sending"), buffer));
	...
	public synchronized void send(byte[] buffer, String ip, int port, boolean isBroadCast) {
	...
	/*
	 * with out inetAddress in Datagram constructor public synchronized void
	 * send(byte[] buffer, InetAddress ip, int port, boolean isBroadCast) {
	 * 
	 * DatagramSocket datagramSocket;
	 * 
	 ...
	 * VPXUtilities.getCurrentTime(0), VPXSessionManager.getCurrentIP(),
	 * ip.getHostAddress(), "UDP", port, buffer.length, new JLabel("Sending"),
	 * buffer));
	 * 
	 * datagramSocket.send(packet);
	 * 
	...
	public synchronized void send(byte[] buffer, InetAddress ip, int port, boolean isBroadCast) {
	...
	public synchronized void setMemory(String ip, int core, long fromAddress, int memindex, int typeSize, int length,
			long newValue) {
	...
	public synchronized void setMemory(String ip, int core, long fromAddress, int memindex, int typeSize, int length,
		...
		try {
			...
			for (int i = 0; i < newValue.length; i++) {
				msg.params.memoryinfo.buffer[i].set(newValue[i]);
			}
	...
	public synchronized void readMemory(MemoryViewFilter filter) {
		...
		try {
			...
			DatagramPacket packet = new DatagramPacket(buffer, buffer.length,
					InetAddress.getByName(filter.getProcessor()), currentCommPort);

			VPXNetworkLogger.updatePacket(new VPXNWPacket(0, VPXUtilities.getCurrentTime(0),
					VPXSessionManager.getCurrentIP(), filter.getProcessor(), "UDP", currentCommPort, buffer.length,
					new JLabel("Sending"), buffer));
	...
	public synchronized void populateMemory(String ip, ATPCommand msg) {
		...
		try {
			...
			if (isComplete) {
				...
				parent.populateMemory(ip, index, msg.params.memoryinfo.address.get(),
	...
	public synchronized void readPlot(MemoryViewFilter filter) {
	...
	public synchronized void readPlot(MemoryViewFilter filter1, MemoryViewFilter filter2) {
		...
		try {
			...
			DatagramPacket packet = new DatagramPacket(buffer, buffer.length,
					InetAddress.getByName(filter1.getProcessor()), currentCommPort);

			VPXNetworkLogger.updatePacket(new VPXNWPacket(0, VPXUtilities.getCurrentTime(0),
					VPXSessionManager.getCurrentIP(), filter1.getProcessor(), "UDP", currentCommPort, buffer.length,
					new JLabel("Sending"), buffer));
			...
			VPXNetworkLogger.updatePacket(new VPXNWPacket(0, VPXUtilities.getCurrentTime(0),
					VPXSessionManager.getCurrentIP(), filter2.getProcessor(), "UDP", currentCommPort, buffer.length,
					new JLabel("Sending"), buffer));
	...
	public synchronized void populatePlot(String ip, ATPCommand msg) {
	...
	public synchronized void readSpectrum(String ip, int core, int id) {
	...
	public synchronized void setSpectrumInterrupted(String ip) {
	...
	public synchronized void setSpectrumInterrupted(String ip, int core) {
	...
	public synchronized void populateSpectrum(String ip, ATPCommand msg) {
		...
		try {
			...
			parent.populateSpectrum(ip, (int) msg.params.memoryinfo.core.get(), index, fl);
	...
	public synchronized void populateBISTResult(String ip, ATPCommand msg) {
	...
	public synchronized void populateBISTResult(String ip, BISTCommand msg) {
	...
	class VPXMessageConsoleMonitor implements Runnable {
		...
		public VPXMessageConsoleMonitor() {

			try {

				if (inetAddressRecv != null)
					messageReceiverSocket = new DatagramSocket(currentMsgPort, inetAddressRecv);
				else
					messageReceiverSocket = new DatagramSocket(currentMsgPort);
			} catch (Exception e) {
				e.printStackTrace();
		...
		public void run() {
			...
			while (isRunning) {
				...
				try {
					...
					if (messageReceiverSocket != null && !messageReceiverSocket.isClosed()) {
	...
	class VPXCommMonitor implements Runnable {
		...
		public VPXCommMonitor() {
			try {
				if (inetAddressRecv != null)
					communicationSocket = new DatagramSocket(currentCommPort, inetAddressRecv);
				else
					communicationSocket = new DatagramSocket(currentCommPort);
			} catch (Exception e) {
				e.printStackTrace();
	...
	class VPXAdvMonitor implements Runnable {
		...
		public VPXAdvMonitor() {
			...
			try {
				...
				if (inetAddressRecv != null)
					advertisementSocket = new DatagramSocket(currentAdvPort, inetAddressRecv);
				else
					advertisementSocket = new DatagramSocket(currentAdvPort);
		...
		public void run() {
			...
			while (isRunning) {
				...
				try {
					...
					if (advertisementSocket != null && !advertisementSocket.isClosed()) {
	...
	public void printBytes(byte byteVal, int i) {
	...
	public void addBytes(byte byteVal, int i) {
