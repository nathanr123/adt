package com.cti.vpx.controls.graph.utilities.ui.graphs.scrollingline;
...
import com.cti.vpx.controls.graph.utilities.ui.graphs.lineGraph.AbstractLineGraph;
...
public class XYScrollingLineGraph extends AbstractLineGraph implements ILocaleListener {
	...
	private static final long serialVersionUID = 6481530346494962543L;
	...
	private void paintLine(Graphics2D g, final int currentHeight_pixels, final double yPlotRatio,
			final double xPlotRatio, LineState state) {
		if (state.visible) {
	...
	public void addLineValue(Object key, double xValue, double yValue) {
		synchronized (lineMap) {
			...
			if (!state.data.isEmpty() && xValue <= state.data.getFirst().x) {
				throw new IllegalStateException("cannot add an X value <= than the previous X value " + xValue + "<="
						+ state.data.getFirst().x);
			}
	...
	/**
	 * Multiple graphs can be drawn on this DrawSurface. Each graph or signal
	 * must be able to be made in/visible upon request.
	 * 
	 * @param visible
	 *            - True to see the signal, false to make it invisible
	 */
