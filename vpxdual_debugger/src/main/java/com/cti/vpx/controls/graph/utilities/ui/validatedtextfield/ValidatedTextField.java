 ...
 * 
 * This component acts mostly like a JFormattedTextField, but without the
 * horrible complexity, and makes it a lot easier to code text fields that need
 * validation.
 * 
 * Notes: Yes, this is perhaps a strange way of solving the problem of having a
 * TextField where I can validate the contents such that getValue() never
 * returns me bad data, and it deals nicely with focus traversal, etc. But it's
 * also the same way that JSpinner.NumberEditor solves this problem, so I don't
 * feel so bad.
 */
public class ValidatedTextField extends JComponent {

	private static final long serialVersionUID = 4275955676805352157L;

	/**
	 * an example of the null-object pattern, so I don't have to test the model
	 * field for null.
	 */
		...
		public String objectToText(Object obj) {
			if (obj == null) {
				...
				throw new IllegalStateException(
						"cannot set a value on the default model - must configure ValidatedTextField with model first.");
			}
		}
	...
	public ValidatedTextField() {
		...
		// on losing focus, we do the equivalent of the
		// JFormattedFieldText.COMMIT_OR_REVERT behaviour
		...
		// this fires when the contents of the field are "accepted" e.g. when
		...
		// implement Cancel semantics like JFormattedTextField i.e. revert the
		// contents when the user presses ESCAPE
	...
	private class CancelAction extends TextAction {
		 ...
		 * 
		 */
		private static final long serialVersionUID = 3583881427489397410L;
