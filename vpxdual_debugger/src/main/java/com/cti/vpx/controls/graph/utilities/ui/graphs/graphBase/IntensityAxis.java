...
import com.cti.vpx.controls.graph.utilities.ui.graphs.axisscale.NumberAxisScale;
import com.cti.vpx.controls.graph.utilities.ui.images.IconManager;

/**
 * An axis component that draws a gradient-shaded background to indicate the
 * "intensity" of the value at that point.
 */
public class IntensityAxis extends JPanel implements ILocaleListener {
	/**
	 * 
	 */
	private static final long serialVersionUID = 2846224518015550265L;
	...
	/**
	 * Creates a new instance of cIntensityAxis. This will be a Y-Axis.
	 */
	public IntensityAxis() {
		...
		oWaterfallOnOffButton.setIcon(IconManager
				.getSizedFor("/com/cti/vpx/controls/graph/utilities/ui/images/colour_waterfall_off.png", this, 1.2f));
		oWaterfallOnOffButton.setSelectedIcon(IconManager
				.getSizedFor("/com/cti/vpx/controls/graph/utilities/ui/images/colour_waterfall.png", this, 1.2f));
		...
		// the number format is used to size the width, so make it shorter to
		// reduce the width
	...
	public void setThresholdLimits(float fMinThresholdLimit, float fMaxThresholdLimit) {
		if (this.fMinThresholdLimit == fMinThresholdLimit && this.fMaxThresholdLimit == fMaxThresholdLimit) {
			return;
		}
		...
		if (fMinThresholdLimit > fMinThresholdValue || fMaxThresholdLimit < fMaxThresholdValue) {
			setThresholdValues(Math.max(fMinThresholdLimit, fMinThresholdValue),
	...
	public void setThresholdValues(float fMinThresholdValue, float fMaxThresholdValue) {
		if (this.fMinThresholdValue == fMinThresholdValue && this.fMaxThresholdValue == fMaxThresholdValue) {
			return;
		}
		if (fMinThresholdValue < fMinThresholdLimit)
			fMinThresholdValue = fMinThresholdLimit;
		this.fMinThresholdValue = fMinThresholdValue;
		if (fMaxThresholdValue > fMaxThresholdLimit)
			fMaxThresholdValue = fMaxThresholdLimit;
		this.fMaxThresholdValue = fMaxThresholdValue;

		final int iComponentHeight = axis.getHeight();

		iMinCompPositionY = PixelUnitConverter.unitToPixel(false, fMinThresholdValue, 0, iComponentHeight - 1,
				fMinThresholdLimit, fMaxThresholdLimit);
		iMaxCompPositionY = PixelUnitConverter.unitToPixel(false, fMaxThresholdValue, 0, iComponentHeight - 1,
				fMinThresholdLimit, fMaxThresholdLimit);
	...
	/**
	 * Sets the colors that get used for the gradient when in monochrome (i.e.
	 * not color) mode.
	 */
	...
	/**
	 * Convert amplitude data into an array of RGB data. Mostly used to feed
	 * data into images.
	 */
	public void convertAmplitudeToLine(int[] aiImageData, float[] afAmplitudeFrame_dBm) {
		if (!bColorScale) {
			final PixelUnitConverter.UnitToPixel2 converter = new PixelUnitConverter.UnitToPixel2(true, 0, 255,
	...
	/**
	 * @param awAmplitude_cdBm
	 *            amplitudes in hundredth's of a dBm
	 */
	...
	/**
	 * Convert a floating point value in the intensity range to a greyscale RGB
	 * value.
	 */
	...
	private static final Color[] RAINBOW = new Color[] { new Color(0, 0, 0), new Color(48, 48, 96),
			new Color(0, 64, 248), new Color(0, 136, 248), new Color(0, 216, 248), new Color(0, 248, 208),
			new Color(0, 248, 112), new Color(136, 248, 24), new Color(200, 248, 8), new Color(240, 248, 8),
			new Color(248, 224, 8), new Color(248, 184, 8), new Color(248, 152, 8), new Color(248, 96, 8),
			new Color(248, 0, 0), };
	...
	/**
	 * If this Axis is self maintained it will calculate and draw it's own
	 * labels. The default is false, which means that no labels are drawn.
	 */
	public void setSelfMaintained(boolean bSelfMaintained) {
		...
		if (bSelfMaintained) {
			...
			// the number format is used to size the width, so make it shorter
			// to reduce the width
			axis.setFormat(new DecimalFormat("0"));
		}
		repaint();
	...
	private void calculateLabels() {
		if (bSelfMaintained) {
			final int iComponentHeight = axis.getHeight();
			axis.clear();
			final double dYStepSize_Hz = calculateStepSize(iComponentHeight / 40, fMinThresholdLimit,
					fMaxThresholdLimit);
			...
			for (int i = 0; i < iNumYSteps + 1; i++) {
				...
				final int iPosition = PixelUnitConverter.unitToPixel(false, val, 0, getHeight() - 1, fMinThresholdLimit,
						fMaxThresholdLimit);
	 ...
	 *
	 * @param approxSpaces
	 *            - The number of spaces that would look nice for a particular
	 *            size of drawSurface
	 * @param max
	 *            - The maximum value on the axis
	 * @param min
	 *            - The minimum value on the axis
	 * @return - The step size across the axis in the same unit as arguments min
	 *         & max
	 */
	...
	private final class MyAxis extends NumberAxisScale {
		...
		private static final long serialVersionUID = -4751664131981978710L;
		...
		public void paint(Graphics oGraphics) {
			...
			if (!bColorScale) {
				...
				GradientPaint oGradientPaint = new GradientPaint(0, iMaxCompPositionY, oGradientStartColor, 0,
						iMinCompPositionY, oGradientStopColor);
				...
				for (int i = 0; i < (RAINBOW.length - 1); i++) {
					// The calculations here are important to avoid rounding
					// errors which could leave us
					...
					GradientPaint oGradientPaint = new GradientPaint(0, startRegion, RAINBOW[rainbowIdx], 0, endRegion,
							RAINBOW[rainbowIdx - 1]);
					o2DG.setPaint(oGradientPaint);
			...
			if (iSliderLabelState == SLIDER_LABEL_MIN) {
				o2DG.drawString(axis.formatValueAsLabel(fMinThresholdValue), 1,
						iMinCompPositionY - (SLIDER_LINE_HEIGHT / 2) - 1);
			} else if (iSliderLabelState == SLIDER_LABEL_MAX) {
				o2DG.drawString(axis.formatValueAsLabel(fMaxThresholdValue), 1,
						iMaxCompPositionY - (SLIDER_LINE_HEIGHT / 2) - 1);
			}
	...
	private class AxisMouseHandling implements MouseListener, MouseMotionListener {
		...
		public void mouseDragged(MouseEvent mouseEvent) {
			...
			if (state == STATE_MIN_SLIDER_SELECTED) {
				...
				fMinThresholdValue = (float) PixelUnitConverter.pixelToUnit(false, iMinCompPositionY, 0,
						iComponentHeight - 1, fMinThresholdLimit, fMaxThresholdLimit);
				...
				fMaxThresholdValue = (float) PixelUnitConverter.pixelToUnit(false, iMaxCompPositionY, 0,
						iComponentHeight - 1, fMinThresholdLimit, fMaxThresholdLimit);
			}
			axis.repaint();
		}
