package com.cti.vpx.controls.graph.utilities.fft;

import java.util.ArrayList;
import java.util.List;

import com.cti.vpx.controls.graph.utilities.ui.graphs.lineGraph.SingleLineGraph;

/**
 * This class performs an FFT on incoming data, and then displays the data as a
 * graph.
 */
public class FFTGraph extends SingleLineGraph {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 6612269503312011723L;
	...
	/**
	 * Set the number of frames to ignore before doing an FFT. Used to limit the
	 * CPU usage when data rates are high.
	 */
	...
	/**
	 * @param input
	 *            an array containing real and imaginary value alternating (ie,
	 *            [real] [imag] [real] [imag]...)
	 */
	public void computeFFT(short[] input) {
		...
		if ((inputLen % 2) == 1) {
			throw new IllegalArgumentException(
					"the sample counts for the reals, differs from the counts for imaginaries");
		}
		...
		if (oFFT == null) {
			...
			// if it's currently filling up data, only change oFFT size when the
			// filling-up process finishes.
			// (otherwise oFFT will start at a random point which may contain
			// discontinuity.)
		...
		for (int i = 0; i < sampleSize; i++) {
			...
			if (isBufferFull) {
				...
				// if we exceed the max data rate, we start dumping frames to
				// keep the CPU usage reasonable
				++dumpingCounter;
	...
	private void doFFT(final int currentResolution) {
		...
		// This is just an estimate of the difference between PXGViewer and
		// PXGAnalysis
		final double lostEnergy = 6.87;

		/*
		 * Calculate oFFT and get the spectra. Returning a temporary array that
		 * will be reused for next calculation, in order to avoid the
		 * unnecessary array-creation. Hence, don't modify and/or store the
		 * resultant array; rather copy the values then process the copied
		 * values.
		 */
