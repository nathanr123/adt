package com.cti.vpx.controls.graph.utilities.ui.graphs.hopperHistogram.dualChannel;
...
class DrawSurface extends JPanel
{
	...
	public void drawVerticalGrid(Graphics2D g2d)
	{
		...
		else
		{
			...
			for (int i = 0; i < numXSteps + 1; i++)
			{
				...
				x1 = x2 = PixelUnitConverter.unitToPixel(true, firstValue_Hz + ((i + 1) * xStepSize_Hz), 0,
						getWidth(), dMinX, dMaxX);
			}
	 ...
	 */
	public synchronized void setAxisRanges(double dXAxisMinimum, double dXAxisMaximum,
			double dYAxisMinimum, double dYAxisMaximum)
	{
	...
	public synchronized void setXAxisRange(double dXAxisMinimum, double dXAxisMaximum)
	{
		...
		// make sure the axes are on a power-of-10 boundary
