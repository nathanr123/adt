...
public class FFT {
	...
	/**
	 * Similar to calculate(double[] rdata, double[] idata), but rely on the
	 * external program to directly manipulate the internal arrays to save the
	 * time on creating arrays and moving the data around.
	 * 
	 * @return an array of spectrum data. This array will be reused for next
	 *         calculation, in order to avoid the unnecessary array-creation.
	...
	private void postFFTProcess(double[] spectra, double postFFTScalingFactor, double lostEnergy) {
		for (int i = 0; i < spectra.length; ++i) {
			...
			// dMagnitude[i] = (Math.log( dMagnitude[i] ) / Math.log( 10 )) *
			// scalingFactor;
	...
	public static final double[] blackmanWindow(final int iSize) {
		...
		for (int n = 0; n < iSize; ++n) {
			...
			// Blackman-Harris: W(n)=0.35875 -0.48829[cos(2*PI*n/(NP-1))]
			// +0.14128[cos(4*PI*n/(NP-1))]
			// -0.01168[cos(6*PI*n/(NP-1))]
			// Blackman: W(n) = 0.42323 -0.49755[cos(2*PI*n/(NP-1))]
			// +0.07922cos[cos(4*PI*n/(NP-1))]
