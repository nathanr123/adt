package com.cti.vpx.controls.graph.utilities.objectpool;
...
/**
 * A relatively simply class designed mainly for supplying arrays to the graph
 * code.
 * 
 * This optimization is mainly useful when we have more than one graph on the
 * screen. It reduces memory usage, and makes better use of the CPU's L2 cache.
 * 
 * As a side effect, is also makes GC a little faster, by reducing the cycling
 * of large memory objects through the various generations in the collector.
 */
public final class GraphObjectPool {

	/**
	 * we only allocate arrays of the largest size the code demands. That way,
	 * we need fewer individual arrays, and since they are mainly being used on
	 * the event thread, these arrays are recycled very quickly, so we typically
	...
	public static synchronized int[] checkOutIntArray(int minSize) {
		if (minSize <= intArraySize) {
			// loop through the pool, looking for an array that has not been
			// GC'ed
	...
	public static synchronized void checkIn(int[] object) {
		...
		// ignore it, was checked out before we increased the size of the arrays
		// in the pool
		if (object.length < intArraySize)
			return;
