package com.cti.vpx.controls.graph.utilities.prefs;
...
public class PreferencesLib {
	 ...
	 *            may be null
	 */
	public static void configure(final IPreferencesNode node, final String prefName, final JFrame frame,
	 ...
	 * Restore any previous stored setting, and configure a save listener
	 */
	public static void configure(final IPreferencesNode node, final String prefName, final JTabbedPane tabbedPane) {
	 ...
	 *            may be null
	 */
	public static void configure(final IPreferencesNode node, final String prefName, final Window window,
			Dimension defaultDimension) {
	...
	public static void load(IPreferencesNode node, String prefName, JCheckBox checkbox, boolean defaultVal) {
		checkbox.setSelected(node.getBoolean(prefName, defaultVal));
	}

	public static void load(IPreferencesNode node, String prefName, JTextField textfield, String defaultVal) {
	...
	/**
	 * add the necessary listeners to save preferences for a JXTable's column
	 * preferences
	 */
	private static void addSavePreferencesListener(final IPreferencesNode node, final String tableName,
			final JTable table) {
	...
	private static void storeJTablePrefs(IPreferencesNode node, final String tableName, final JTable table) {
		for (Enumeration<TableColumn> iter = table.getColumnModel().getColumns(); iter.hasMoreElements();) {
	 ...
	 */
	private static void loadJTablePreferences(IPreferencesNode node, String tableName, final JTable table) {
		for (Enumeration<TableColumn> iter = table.getColumnModel().getColumns(); iter.hasMoreElements();) {
			final TableColumn column = iter.nextElement();
			final int modelIndex = column.getModelIndex();
	...
	/**
	 * Use weak preferences. Since JFrame is a global object, attaching
	 * listeners to it can mean that some of my UI objects will "leak". So use a
	 * weak reference to break this leak.
	 */
	...
	private static void loadTabbedPanePrefs(IPreferencesNode node, String prefName, JTabbedPane tabbedPane) {
	...
	private static void storeTabbedPanePrefs(IPreferencesNode node, String prefName, JTabbedPane tabbedPane) {
	 ...
	 */
	public static void addSavePreferencesListener(final JComponent component, final ActionListener listener) {
		/*
		 * Sanity check to make sure that when I copy code I modify the save
		 * prefs calls.
		 */
		...
		if (parent != null) {
			...
			/*
			 * The first time this component is shown, add a close listener to
			 * the parent JFrame. I can't do this initially, because at the time
			 * addSavePreferencesListener is called, the component may not be
			 * added to a frame yet.
			 */
	 ...
	 * add the necessary listeners to save preferences
	 */
	public static void addSavePreferencesListener(final java.awt.Window component, final ActionListener listener) {
