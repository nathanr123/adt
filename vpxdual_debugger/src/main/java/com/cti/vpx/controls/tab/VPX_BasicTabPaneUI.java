...
public class VPX_BasicTabPaneUI extends BasicTabbedPaneUI {
	...
	public static final ImageIcon ICON_CLOSE_DEFAULT = VPXUtilities.getImageIcon("closeunsel.gif", 12, 12);

	public static final ImageIcon ICON_CLOSE_HOVER = VPXUtilities.getImageIcon("closesel.gif", 14, 14);

	public static final ImageIcon ICON_DETACH_DEFAULT = VPXUtilities.getImageIcon("detach2.gif", 12, 12);

	public static final ImageIcon ICON_DETACH_HOVER = VPXUtilities.getImageIcon("detach2.gif", 14, 14);
	...
	public VPX_BasicTabPaneUI() {
		...
		maxB = new JRadioButton(ICON_DETACH_DEFAULT);

		maxB.setActionCommand("DETACH");
	...
	protected void paintActionButton(Graphics g, int dx, int dy, int status, boolean isOver, JRadioButton button,
			BufferedImage image) {

		button.setBorder(null);

		// closeB.setActionCommand("DETACH");
		// closeB.setActionCommand("CLOSE");
		if (isOver) {
			switch (status) {
			case OVER:
				if (button.getActionCommand().equals("DETACH")) {
					button.setIcon(ICON_DETACH_HOVER);
				} else if (button.getActionCommand().equals("CLOSE")) {
					button.setIcon(ICON_CLOSE_HOVER);
				}
			...
			default:
				...
				if (button.getActionCommand().equals("DETACH")) {
					button.setIcon(ICON_DETACH_DEFAULT);
				} else if (button.getActionCommand().equals("CLOSE")) {
					button.setIcon(ICON_CLOSE_DEFAULT);
				}
					...
					for (int i = 0; i < numChildren; i++) {
						...
						if (child instanceof ScrollableTabViewport) {
							...
							if (totalTabWidth > tw) {
								...
								visible = true;
								bx = dir == EAST ? bounds.width - insets.left - bsize.width
	...
	private class ScrollableTabSupport implements ChangeListener {
		...
		public void stateChanged(ChangeEvent e) {
			...
			scrollForwardButton
					.setEnabled(leadingTabIndex < tabCount - 1 && viewSize.width - viewRect.x > viewRect.width);
