 ...
 * </ul>
 * <p/>
 * The queue records time of the event dispatching start. This time is used by
 * the timers to check if dispatching takes longer than their periods. If so the
 * timers trigger associated actions.
 * <p/>
 * In order to use this queue application should call <code>install()</code>
 * method. This method will create, initialize and register the alternative
 * queue as appropriate. It also will return the instance of the queue for
 * further interactions. Here's an example of how it can be done:
 * <p/>
 * 
 * <pre>
...
public class EventQueueWatchdog extends EventQueue {
	...
	public interface IWatchdogListener {
		 ...
		 * 
		 * @param stacktrace
		 *            contains a stacktrace of the thread that caused the
		 *            problem
		 */
	...
	/**
	 * Install a watchdog with a default timeout of 1 second that writes a
	 * stacktrace to System.out.
	 */
	...
	/**
	 * Install a watchdog. It will print a message on System.out whenever an AWT
	 * thread action takes too long.
	 */
	...
	private class Watchdog extends TimerTask {
		...
		private String toString(StackTraceElement[] stacktrace) {
			...
			for (StackTraceElement element : stacktrace) {
				...
				// format the output carefully so that eclipse can parse and
				// hyperlink
				// to code properly.
				builder.append("\tat ");
