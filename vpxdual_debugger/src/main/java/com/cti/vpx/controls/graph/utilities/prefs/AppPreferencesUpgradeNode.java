 ...
 * 
 * It will write any prefs to the new location, but when reading, will first
 * attempt to read from the new location, and if nothing is found, will then
 * read from the old location.
 */
public class AppPreferencesUpgradeNode implements IPreferencesNode {

	/**
	 * create my own preferences node to store the state of upgrades, to make
	 * sure none of the clients of this class accidentally stomp on my state.
	 */
	private static final IPreferencesNode prefsNode = new AppPreferencesNode(AppPreferencesUpgradeNode.class);

	/*
	 * Explanation of the state changes, because it can be a little tricky to
	 * follow.
	 * 
	 * Start state: UPGRADED flag does not exist Either we have no old or new
	 * data, or we have old data, but it has not been upgraded. We do a read
	 * from the old data, and write to the new data if we find something. As
	 * soon as the first user-write is initiated, we set the UPGRADED to TRUE,
	 * and then start reading and writing exclusively to the new data.
	 * 
	 * Start state: UPGRADED flag==TRUE We read and write from new data,
	 * ignoring old data.
	 */
	 ...
	 * 
	 * @param oldClassName
	 *            if this does not include a full package, we assume that only
	 *            the class name changed, and not the package.
	 * @param newClass
	 */
