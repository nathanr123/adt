...
public class VPX_MemoryBrowserWindow extends JFrame implements WindowListener {
	...
	private void createFilterPanel() {
		...
		mapPanel.setLayout(new MigLayout("", "[109px][46px][406px,grow,fill][89px][pref!]", "[23px]"));
	...
	private String getLengthAsString(int size) {
		...
		if (size == ATP.DATA_TYPE_SIZE_BIT8) {
			...
			if (size == ATP.DATA_TYPE_SIZE_BIT16) {
				
				length = length * 2;
				
			} else
				if (size == ATP.DATA_TYPE_SIZE_BIT32) {
					
					length = length * 4;
					
				} else
					if (size == ATP.DATA_TYPE_SIZE_BIT64) {
	...
	private void doReadMemory() {
		...
		int isAddressValid = isAddressValid();
				...
				if (!isSelectedCoreValid) {
					...
					if (isAddressValid < 0) {
						...
						JOptionPane.showMessageDialog(VPX_MemoryBrowserWindow.this,
								getAddressErrorMessage(isAddressValid), "Validation", JOptionPane.ERROR_MESSAGE);
	...
	private String getAddressErrorMessage(int option) {
		...
		/*
		 * if (cmbCores.getSelectedIndex() > 0) { switch
		 * (cmbCores.getSelectedIndex() - 1) { case 0: ret = String.format(
		 * "Addres is invalid for Core - %d.\nAddress Range between ( 0x%08X and 0x%08X ) or (  0x%08X and 0x%08X  )"
		 * , (cmbCores.getSelectedIndex() - 1), ATP.CORE0_DDR3_START_ADDRESS,
		 * ATP.CORE0_DDR3_END_ADDRESS, ATP.C0_L2SRAM_START_ADDRESS,
		 * ATP.C0_L2SRAM_END_ADDRESS); break; case 1: ret = String.format(
		 * "Addres is invalid for Core - %d.\nAddress Range between ( 0x%08X and 0x%08X ) or (  0x%08X and 0x%08X  )"
		 * , (cmbCores.getSelectedIndex() - 1), ATP.CORE1_DDR3_START_ADDRESS,
		 * ATP.CORE1_DDR3_END_ADDRESS, ATP.C1_L2SRAM_START_ADDRESS,
		 * ATP.C1_L2SRAM_END_ADDRESS); break; case 2: ret = String.format(
		 * "Addres is invalid for Core - %d.\nAddress Range between ( 0x%08X and 0x%08X ) or (  0x%08X and 0x%08X  )"
		 * , (cmbCores.getSelectedIndex() - 1), ATP.CORE2_DDR3_START_ADDRESS,
		 * ATP.CORE2_DDR3_END_ADDRESS, ATP.C2_L2SRAM_START_ADDRESS,
		 * ATP.C2_L2SRAM_END_ADDRESS); break; case 3: ret = String.format(
		 * "Addres is invalid for Core - %d.\nAddress Range between ( 0x%08X and 0x%08X ) or (  0x%08X and 0x%08X  )"
		 * , (cmbCores.getSelectedIndex() - 1), ATP.CORE3_DDR3_START_ADDRESS,
		 * ATP.CORE3_DDR3_END_ADDRESS, ATP.C3_L2SRAM_START_ADDRESS,
		 * ATP.C3_L2SRAM_END_ADDRESS); break; case 4: ret = String.format(
		 * "Addres is invalid for Core - %d.\nAddress Range between ( 0x%08X and 0x%08X ) or (  0x%08X and 0x%08X  )"
		 * , (cmbCores.getSelectedIndex() - 1), ATP.CORE4_DDR3_START_ADDRESS,
		 * ATP.CORE4_DDR3_END_ADDRESS, ATP.C4_L2SRAM_START_ADDRESS,
		 * ATP.C4_L2SRAM_END_ADDRESS); break; case 5: ret = String.format(
		 * "Addres is invalid for Core - %d.\nAddress Range between ( 0x%08X and 0x%08X ) or (  0x%08X and 0x%08X  )"
		 * , (cmbCores.getSelectedIndex() - 1), ATP.CORE5_DDR3_START_ADDRESS,
		 * ATP.CORE5_DDR3_END_ADDRESS, ATP.C5_L2SRAM_START_ADDRESS,
		 * ATP.C5_L2SRAM_END_ADDRESS); break; case 6: ret = String.format(
		 * "Addres is invalid for Core - %d.\nAddress Range between ( 0x%08X and 0x%08X ) or (  0x%08X and 0x%08X  )"
		 * , (cmbCores.getSelectedIndex() - 1), ATP.CORE6_DDR3_START_ADDRESS,
		 * ATP.CORE6_DDR3_END_ADDRESS, ATP.C6_L2SRAM_START_ADDRESS,
		 * ATP.C6_L2SRAM_END_ADDRESS); break; case 7: ret = String.format(
		 * "Addres is invalid for Core - %d.\nAddress Range between ( 0x%08X and 0x%08X ) or (  0x%08X and 0x%08X  )"
		 * , (cmbCores.getSelectedIndex() - 1), ATP.CORE7_DDR3_START_ADDRESS,
		 * ATP.CORE7_DDR3_END_ADDRESS, ATP.C7_L2SRAM_START_ADDRESS,
		 * ATP.C7_L2SRAM_END_ADDRESS); break; } }
		 */
		return ret;
	}
	...
	private int isAddressValid() {
		...
		if (address == -1 || cmbCores.getSelectedIndex() == 0) {
			...
			if (!VPXUtilities.isBetween(address, ATP.DDR3_START_ADDRESS, ATP.DDR3_END_ADDRESS)
				...
				if (!VPXUtilities.isBetween((address + val), ATP.DDR3_START_ADDRESS, ATP.DDR3_END_ADDRESS)
						&& !VPXUtilities.isBetween((address + val), ATP.L2SRAM_START_ADDRESS, ATP.L2SRAM_END_ADDRESS)
						&& !VPXUtilities.isBetween((address + val), ATP.MSMCSRAM_START_ADDRESS,
								ATP.MSMCSRAM_END_ADDRESS)) {
					
					retValue = -3;
				}
			
		/*
		 * int core = cmbCores.getSelectedIndex() - 1; if (core == 0) { if
		 * (!VPXUtilities.isBetween(address, ATP.CORE0_DDR3_START_ADDRESS,
		 * ATP.CORE0_DDR3_END_ADDRESS) && !VPXUtilities.isBetween(address,
		 * ATP.C0_L2SRAM_START_ADDRESS, ATP.C0_L2SRAM_END_ADDRESS)) { retValue =
		 * false; } } else if (core == 1) { if (!VPXUtilities.isBetween(address,
		 * ATP.CORE1_DDR3_START_ADDRESS, ATP.CORE1_DDR3_END_ADDRESS) &&
		 * !VPXUtilities.isBetween(address, ATP.C1_L2SRAM_START_ADDRESS,
		 * ATP.C1_L2SRAM_END_ADDRESS)) { retValue = false; } } else if (core ==
		 * 2) { if (!VPXUtilities.isBetween(address,
		 * ATP.CORE2_DDR3_START_ADDRESS, ATP.CORE2_DDR3_END_ADDRESS) &&
		 * !VPXUtilities.isBetween(address, ATP.C2_L2SRAM_START_ADDRESS,
		 * ATP.C2_L2SRAM_END_ADDRESS)) { retValue = false; } } else if (core ==
		 * 3) { if (!VPXUtilities.isBetween(address,
		 * ATP.CORE3_DDR3_START_ADDRESS, ATP.CORE3_DDR3_END_ADDRESS) &&
		 * !VPXUtilities.isBetween(address, ATP.C3_L2SRAM_START_ADDRESS,
		 * ATP.C3_L2SRAM_END_ADDRESS)) { retValue = false; } } else if (core ==
		 * 4) { if (!VPXUtilities.isBetween(address,
		 * ATP.CORE4_DDR3_START_ADDRESS, ATP.CORE4_DDR3_END_ADDRESS) &&
		 * !VPXUtilities.isBetween(address, ATP.C4_L2SRAM_START_ADDRESS,
		 * ATP.C4_L2SRAM_END_ADDRESS)) { retValue = false; } } else if (core ==
		 * 5) { if (!VPXUtilities.isBetween(address,
		 * ATP.CORE5_DDR3_START_ADDRESS, ATP.CORE5_DDR3_END_ADDRESS) &&
		 * !VPXUtilities.isBetween(address, ATP.C5_L2SRAM_START_ADDRESS,
		 * ATP.C5_L2SRAM_END_ADDRESS)) { retValue = false; } } else if (core ==
		 * 6) { if (!VPXUtilities.isBetween(address,
		 * ATP.CORE6_DDR3_START_ADDRESS, ATP.CORE6_DDR3_END_ADDRESS) &&
		 * !VPXUtilities.isBetween(address, ATP.C6_L2SRAM_START_ADDRESS,
		 * ATP.C6_L2SRAM_END_ADDRESS)) { retValue = false; } } else if (core ==
		 * 7) { if (!VPXUtilities.isBetween(address,
		 * ATP.CORE7_DDR3_START_ADDRESS, ATP.CORE7_DDR3_END_ADDRESS) &&
		 * !VPXUtilities.isBetween(address, ATP.C7_L2SRAM_START_ADDRESS,
		 * ATP.C7_L2SRAM_END_ADDRESS)) { retValue = false; } }
		 */
		return retValue;
	}
