...
import com.cti.vpx.controls.graph.utilities.ui.graphs.graphBase.ZoomAdapter;

/**
 * This is an optimized form of AbstractLineGraph which only handles a single
 * line, which does not have changing X-values. Specifically, this is optimized
 * for drawing FFT-type graphs.
 * 
...
public class SingleLineGraph extends AbstractLineGraph {
	private static final long serialVersionUID = 2945318014384626685L;
	...
	protected void drawGraph(Graphics2D g) {
		...
		synchronized (oCacheLock) {
			if (iCachedWidth != iWidth || iCachedMinX != getMinimumX() || iCachedMaxX != getMaximumX()) {
				bCalculateX = true;
				iCachedWidth = iWidth;
				iCachedMinX = getMinimumX();
				iCachedMaxX = getMaximumX();
			}
			if (bCacheChanged || iCachedHeight != iHeight || iCachedMinY != getMinimumY()
		...
		/*
		 * Only draw the data that is going to be visible. Note: the +2,-2
		 * calculations are to allow for high zoom factors. If we are highly
		 * zoomed in, the first line on the left may start off-screen.
		 */
		final int startIndex = Math.max(0, Math.abs(oLineData.binarySearchXValues(getMinimumX())) - 2);
		// find the index where the maxX would be (the value may not necessarily
		// be in the array, but the index
		// will index where it _would_ be
		int endIndex = Math.abs(oLineData.binarySearchXValues(getMaximumX()));
		// adjust index for high zoom factors where the end-point of a line may
		// line off screen
		endIndex = endIndex + 2;
		...
		/*
		 * if there is a clip region set, take advantage of it and reduce the
		 * amount of data to be drawn
		 */
		if (clip == null) {
	 ...
	 * 
	 * Note that this tells the graph that the Y data has changed, but the graph
	 * will assume that the X data has remained the same.
	 */
	...
	/**
	 * This method will cause the display to clear. Note: this method should be
