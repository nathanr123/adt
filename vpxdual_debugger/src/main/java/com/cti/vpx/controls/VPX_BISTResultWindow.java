			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!lblTestDetailP2020IPVal.getText().trim().equals(BIST.NA)) {
					...
					if (option == JOptionPane.YES_OPTION) {
						...
						// parent.setReboot(lblTestDetailP2020IPVal.getText().trim(),
						// 0, ATP.FLASH_DEVICE_NAND, 0);
						VPX_BISTResultWindow.this.dispose();
						
						parent.showBootOption(lblTestDetailP2020IPVal.getText().trim());
					}
				} else {
		...
		try {
			...
			if (chooser.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
				...
				String fileName = String.format("%s/%s_%s_%s.txt",
						VPXUtilities.getPathAsLinuxStandard(chooser.getSelectedFile().getPath()),
			...
			if (value == -1) {
				...
				if (pType == PROCESSOR_LIST.PROCESSOR_P2020) {
					...
					if (pType == PROCESSOR_LIST.PROCESSOR_DSP1) {
						...
						lblExitingApplication.setText(
								"<html>Built in Self Testing in progress...<br><br>DSP1 Test Completed</html>");
						
					} else
						if (pType == PROCESSOR_LIST.PROCESSOR_DSP2) {
							
							lblExitingApplication.setText(
									"<html>Built in Self Testing in progress...<br><br>DSP2 Test Completed</html>");
						}
	...
	public void windowClosing(WindowEvent e) {
		...
		if (!lblTestDetailP2020IPVal.getText().trim().equals(BIST.NA)) {
			...
			if (option == JOptionPane.YES_OPTION) {
				...
				// parent.setReboot(lblTestDetailP2020IPVal.getText().trim(), 0,
				// ATP.FLASH_DEVICE_NAND, 0);
